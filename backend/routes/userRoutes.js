...
router.post('/reg-new-user', async(req, res) => {
    ...
    try {
        ...
        if(error) return res.status(400).json({success: false, message: "User registration failed"});
...
router.get('/view-user-profile/:user_id', async(req, res) => {
    ...
    try {
        ...
        if(error) return res.status(400).json({success: false, message: "Failed to fetch user record"});
...
router.put('/update-unique-user/:user_id/update_value', async(req, res) => {
    ...
    try {
        ...
        if(error) return res.status(400).json({success: false, message: "User update failed"});
        return res.status(201).json({success: true, message: "User update successful", data: data})
    } catch (error) {
        return res.status(500).json({success: false, message: "Server error"});
    }
});
